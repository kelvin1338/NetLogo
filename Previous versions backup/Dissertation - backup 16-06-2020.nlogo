...
;  ;We want to begin with some infected humans
;  ask n-of infected-cases humans [
;    set infected? true
;    set color yellow
;  ]
end
...
to setup
  ...
  let initial_infected_humans round (count humans * (initial_infected_proportion / 100))
...
to infect
  if (not infected_previously?) [ ;only applies to people not infected
    ...
    if number_of_infected_around > 0 [ ;if there are infected people around
      ...
      if (infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance [   ;infected according to infectivity + within distance
        get-infected
      ]
    ]
  ]
