...
to setup-globals
  ...
  set mask_penetration_rate 0.15
end
...
to infect
  if (not infected_previously?) [ ;only applies to people not infected
    ...
    let infected_around people_around with [infected? = true] ;let "infected_around" be the "people_around" with infection
    let number_of_infected_around count infected_around
    if number_of_infected_around > 0 [ ;if there are infected people around
      ...
      ifelse (not wearmask?) [if (infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance   ;infected according to infectivity + within distance
        [get-infected] ;if victim himself does not wear mask, infection rate not affected
      ]
      [ if (mask_penetration_rate * infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance   ;infected according to infectivity + within distance
        [get-infected] ;if victim himself wears mask, infection rate reduced according to the mask penetration rate
      ]
