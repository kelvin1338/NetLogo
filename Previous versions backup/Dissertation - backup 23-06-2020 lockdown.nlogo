...
to setup
  ...
  let initial_lockdown_disobeyers round (count humans * (Disobey_Lockdown / 100))
...
to move-forward-randomly
  ifelse (not feel_symptoms?) [ ;people only move if no symptoms
    ...
    ifelse Total_lockdown? [ ;if no symptoms and total lockdown, don't move, except for the ones disobeying
      if disobey_lockdown? [
        forward random-float 0.25 ;only disobeying ones move
      ]
    ]
    [
      forward random-float 0.25 ;if no symptoms and no lockdown, everyone moves
    ]
  ]
...
to infect
  if (not infected_previously?) [ ;only applies to people not infected
    ...
    [let infected_around people_around with [infected? = true]
     ...
     if number_of_infected_around > 0 [ ;if there are infected no-mask people around
