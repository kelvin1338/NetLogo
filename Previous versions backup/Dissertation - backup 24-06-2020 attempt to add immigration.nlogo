...
globals [
  ...
  hour_of_day ;resets back to 1 when ticker reaches to 24
...
to setup
  ...
  ;let people_exiting round (count humans * (People_exiting_city / 100))
  reset-ticks
end
...
to exit_city_daily [#people_exiting]
  if (count (humans with [not feel_symptoms?]) > #people_exiting) [
    ask n-of #people_exiting humans with [not feel_symptoms?] [die]
  ]
end

to enter_city_daily [#people_entering #entering_infected_percentage]
  let num_of_entries_age_cat (#people_entering / 9)
  setup-agents num_of_entries_age_cat age_cat_0-9
  setup-agents num_of_entries_age_cat age_cat_10-19
  setup-agents num_of_entries_age_cat age_cat_20-29
  setup-agents num_of_entries_age_cat age_cat_30-39
  setup-agents num_of_entries_age_cat age_cat_40-49
  setup-agents num_of_entries_age_cat age_cat_50-59
  setup-agents num_of_entries_age_cat age_cat_60-69
  setup-agents num_of_entries_age_cat age_cat_70-79
  setup-agents num_of_entries_age_cat age_cat_80+
  infect_people #people_entering * (#entering_infected_percentage / 100)
end
...
to go
  ...
  ifelse hour_of_day >= 24 [
    let travellers traveller_count
    if not close_borders? [
      ;exit_city_daily travellers
      enter_city_daily travellers infected_travellers
    ]
    set hour_of_day 1
  ]
  [
...
1238
207
During lockdown, individuals are not allowed to move, except for essential workers
10
0.0
1
