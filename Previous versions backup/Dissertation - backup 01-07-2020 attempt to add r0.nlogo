...
to setup-agents [#total-humans #age_cat]
  create-humans #total-humans [
    set initially_susceptible? true
...
to setup
  ...
  ask humans with [infected?] [set initially_susceptible? false]
...
to go
  ...
  ask humans [clear-count]
  ask humans [infect]
  ask humans [infection_aftermath]
  ask humans [calculate-r0]
...
to move-forward-randomly ;now we have: essential_worker, intracity_travel,
  ifelse (not feel_symptoms?) [ ;people only move if no symptoms
    ifelse Total_lockdown? [ ;if no symptoms and total lockdown, don't move, except for the ones disobeying and essential workers
      ...
      if intracity_travel? [
        ...
        if confine_city_travel? [
          if city != [city] of home_patch [face home_patch forward 1]   ;lockdown disobeyers move randomly within city
        ]
      ]
    ...
    [ ;if there is no lockdown...
      if intracity_travel? [ ;intracity travellers move quicker, but does not leave the vicinity of the city
        ifelse coin-flip? [right random 45] [left random 45]
        forward random-float 0.2
        if confine_city_travel? [
...
to calculate-r0
  ...
  let new-infected sum [ nb-infected ] of humans
  let new-recovered sum [ nb-recovered ] of humans
...
;    count turtles with [ cured? ]
  ...
  let s0 count humans with [ initially_susceptible? ]
  ...
  if total_population - (count humans with [not infected? and not infected_previously?]) != 0 and (count humans with [not infected? and not infected_previously?]) != 0
  [
    ;; This is derived from integrating dI / dS = (beta*SI - gamma*I) / (-beta*SI):
    set r0 (ln (s0 / (count humans with [not infected? and not infected_previously?])) / (total_population - (count humans with [not infected? and not infected_previously?])))
