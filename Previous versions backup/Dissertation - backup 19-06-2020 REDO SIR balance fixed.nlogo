...
to setup-globals
  ...
  set mask_penetration_rate (mask_particle_penetration / 100)
end
...
to infect
  if (not infected_previously?) [ ;only applies to people not infected
    ...
    let infected_around_nomask people_around with [infected? = true and (not wearmask?)] ;let "infected_around" be the "people_around" with infection
    ...
    if number_of_infected_around_nomask > 0 [ ;if there are infected people around
      ...
      ifelse (not wearmask?) [if (infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance   ;infected according to infectivity + within distance
        [get-infected]
      ]
      ...
      let within_infectious_distance (random(metres_per_patch) + 1) ;define infectious distance
      set within_infectious_distance within_infectious_distance + random-float ( social_distancing_metres )
      ifelse (not wearmask?) [if ((mask_penetration_rate) * infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance   ;infected according to infectivity + within distance
        [get-infected] ;we multiply by mask penetration rate because victim is already wearing mask
      ]
      [ if ((mask_penetration_rate) * (mask_penetration_rate) * infectivity >= (random(100) + 1)) and within_infectious_distance <= maximum_infectious_distance   ;infected according to infectivity + within distance
        [get-infected] ;we multiply by mask penetration rate twice because victim and infector is wearing mask; therefore two layers of masks
      ]
    ]
...
to get-infected
  ...
  set symptom_delay_duration 24 * (random-normal days_before_symptom_showing 3.5) ;avg days before symptom shows
