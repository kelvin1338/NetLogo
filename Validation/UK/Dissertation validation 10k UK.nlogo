...
to go
  ...
  if (ticks mod (7 * 24) = 0) and (ticks <= (15 * 24)) [
  infect_people 40
  ]

  if (ticks mod (7 * 24) = 0) and ((ticks > 15 * 24) and (ticks < 38 * 24)) [
    infect_people 6
  ]

  if (ticks mod (7 * 24) = 0) and (ticks >= 38 * 24) [
    infect_people 18
  ]

  if ticks = (3 * 24) [
    set symptomatic_isolation_rate 60
  ]

  if ticks = (7 * 24) [
    set symptomatic_isolation_rate 90
  ]

  if ticks = (10 * 24) [
    set social_distancing_metres 2
    ]

  if ticks = (15 * 24) [
    set total_lockdown? true
  ]

  if ticks = (28 * 24) [
    set social_distancing_metres 1.5
  ]

  if ticks = (52 * 24) [
    set social_distancing_metres 0.5
    set symptomatic_isolation_rate 80
...
to get-infected
  ...
  set symptom_delay_duration 24 * (random-normal days_before_symptom_showing 2) ;duration (converted to ticks or hours) before symptoms show
...
638
% Currently infected
100 * (count humans with [infected?] / (count humans))
4
1
11
...
1565
134
Only adjust below if above is enabled
10
0.0
1
...
86
Force-outbreak
infect_people round (0.25 * count humans * (initial_infected_proportion / 100))\n
NIL
1
T
OBSERVER
...
<experiments>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="lockdown_delay">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="social_distancing_metres">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="symptomatic_isolation_rate">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pop_10-19">
      <value value="1121"/>
